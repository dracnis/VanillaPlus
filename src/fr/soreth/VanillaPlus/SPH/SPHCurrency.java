...
public class SPHCurrency extends SPH{
	private static Pattern pattern = Pattern.compile("%" + sender + "_currency_([0-9]+)%");
	private static List<SPHCurrency> instance = new LinkedList<>();
	...
	private static SPHCurrency getInstance(short currency) {
		for(SPHCurrency sphc : instance) {
			if(sphc.currency == currency)
				return sphc;
		}
		SPHCurrency sphta = new SPHCurrency();
	...
	public String getReplacement(Localizer lang, VPSender sender){
		...
		return currency.format(amount) + " " + currency.getName(amount).getMessage(lang);
	}
}
